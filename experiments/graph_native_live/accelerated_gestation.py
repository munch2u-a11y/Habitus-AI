...
def add_episode(
    ...
    record = MemoryRecord(
        ...
        metadata={
            ...
            "causal_trunk": episode.input_trunk.value,
        },
    )
...
def cross_modal_language_schooling(
    ...
    candidates = [
        cluster
        for cluster in clusters(mind)
        if cluster.child_node_id is not None and cluster.semantic_node_id is None
    ]
    for index, cluster in enumerate(sorted(candidates, key=lambda item: item.cluster_id)):
        ...
        topics = Counter(str(record.metadata.get("curriculum_topic", "")) for record in records)
        categories = Counter(
            str(record.metadata.get("curriculum_category", "")) for record in records
        )
        outputs = Counter(str(record.metadata.get("output_trunk", "LOOK")) for record in records)
        ...
        record = MemoryRecord(
            ...
            metadata={
                ...
                "membrane_words": True,
                "causal_trunk": InputTrunk.HEAR.value,
            },
        )
        mind.store.add_record(record)
        ...
        semantic = mind.graph.attach_semantic_port(
            child_id,
            record,
            pulse=pulse,
            terms=(topic,),
        ...
        if trace is None:
            raise RuntimeError(
                f"language schooling did not reach {semantic.concept_id}"
            )
        mind.graph.deposit_trace(
            record,
            trace,
            pulse=pulse,
...
def grow_assembly(
    ...
    *,
    ...
    for repetition in range(3):
        ...
        record = MemoryRecord(
            ...
            metadata={
                ...
                "membrane_words": False,
                "causal_trunk": InputTrunk.NOTICE.value,
            },
        )
        mind.store.add_record(record)
        ...
        child_id = mind.graph.stage_growth(
            record,
    ...
    cluster = next(
        ...
        if cluster.child_node_id == child_id
    )
    child_id = str(cluster.child_node_id)
    pulse, _ = mind._next_pulse()
    label_record = MemoryRecord(
        record_id=f"assembly:label-record:{level}:{name}",
        event_id=f"assembly:label-event:{level}:{name}",
        record_type=RecordType.INBOUND_MESSAGE,
        source_id="developmental_caregiver",
        timestamp=utc_now(),
        text=f"I use the spoken label {name} for this broader relation.",
        embedding=as_tuple(centroid),
        provenance={
            "origin": "accelerated_gestation",
            "seeded_structure": False,
            "formation": "recursive_language_schooling",
        },
        metadata={
            "experience_id": f"assembly:label:{level}:{name}",
            "preference": preference,
            "preference_confidence": 0.9,
            "curriculum_topic": f"assembly:{name}",
            "curriculum_category": f"assembly-level-{level}",
    ...
    mind.store.add_record(label_record)
    hear_parent = mind.graph.deposit_experience(
        label_record,
        input_trunk=InputTrunk.HEAR,
        pulse=pulse,
    ...
    mind.graph.add_relation(
        ...
        side=GraphSide.INPUT,
        pulse=pulse,
        evidence_record_ids=(label_record.record_id,),
    )
    semantic = mind.graph.attach_semantic_port(
        child_id,
        label_record,
    ...
    semantic_id = semantic.concept_id
...
def compile_mind(
    ...
    *,
    ...
    with BaseAgenticMemoryRAG(
        ...
        language_schooling = cross_modal_language_schooling(mind)
