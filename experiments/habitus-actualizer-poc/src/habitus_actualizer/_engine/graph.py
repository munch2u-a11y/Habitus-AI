...
class GraphRuntime:
    ...
    def matching_child_traces(
        ...
        *,
        ...
        pulse_id: str,
        limit: int = 2,
    ) -> list[tuple[str, TraversalTrace, float]]:
        """Find the deepest learned nonverbal paths compatible with a record."""
        experience_id = self._experience_id(record)
        state = self.store.get_experience_state(experience_id)
        preference = state.preference_mean if state else 0.0
        candidates: list[tuple[str, TraversalTrace, float]] = []
        for child in self.store.list_concepts(kind="child"):
            cluster = self.store.overlap_cluster_for_child(child.concept_id)
            if cluster is None:
                continue
            ...
            if similarity < self.growth_overlap_threshold:
                continue
            trace = self.traverse(
                pulse_id=pulse_id,
                side=GraphSide.INPUT,
                target_id=child.concept_id,
                endpoint_score=similarity,
                required_input_trunk=input_trunk,
                mark_active=False,
            )
            if trace is not None:
                candidates.append((child.concept_id, trace, similarity))
        candidates.sort(
            key=lambda item: (-len(item[1].path_node_ids), -item[2], item[0])
        )
        # Once a deeper concept matches, do not also treat its ancestors as
        # independent co-activations. That would add shortcut edges and flatten
        # the very hierarchy recursive growth is meant to preserve.
        input_edges = self.store.list_edges(GraphSide.INPUT)
        ...
        selected: list[tuple[str, TraversalTrace, float]] = []
        for candidate in candidates:
            if any(reaches(candidate[0], existing[0]) for existing in selected):
                continue
    ...
    def stage_growth(
        ...
        *,
        ...
        parent_trace = self.traverse(
            pulse_id=f"growth-depth:{pulse}",
            side=GraphSide.INPUT,
            target_id=parent_id,
            endpoint_score=1.0,
            required_input_trunk=input_trunk,
        ...
        child_layer = len(parent_trace.path_node_ids) if parent_trace is not None else 3
        semantic_edge = (
            self._ensure_edge(
                GraphSide.INPUT,
                child_id,
