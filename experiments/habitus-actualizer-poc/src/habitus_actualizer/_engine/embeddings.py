...
def structured_feature_embedding(
    features: Iterable[str],
    dimension: int,
    *,
    namespace: str = "sensory-features",
    hashes_per_feature: int = 4,
) -> list[float]:
    """Encode shared non-language sensor features into a comparable direction.

    Unlike ``opaque_payload_embedding``, repeated feature IDs deliberately share
    coordinates. The feature IDs remain backend symbols; they never become
    membrane words or model-facing context.
    """
    if dimension < 16:
        raise ValueError("embedding dimension must be at least 16")
    resolved = tuple(dict.fromkeys(str(item).strip() for item in features if str(item).strip()))
    if not resolved:
        raise ValueError("at least one sensory feature is required")
    vector = [0.0] * int(dimension)
    repeats = max(1, int(hashes_per_feature))
    for feature in resolved:
        material = f"{namespace}\0{feature}".encode("utf-8")
        for counter in range(repeats):
            digest = hashlib.sha256(material + counter.to_bytes(2, "big")).digest()
            index = int.from_bytes(digest[:8], "big") % dimension
            sign = 1.0 if digest[8] & 1 else -1.0
            vector[index] += sign
    norm = math.sqrt(sum(value * value for value in vector)) or 1.0
