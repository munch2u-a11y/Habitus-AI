...
from .context import render_verified_tool_result
...
class RetrievalEngine:
    def __init__(
        ...
        *,
        ...
        self.lexical_top_k = max(0, lexical_top_k)
        self.verified_result_top_k = max(0, verified_result_top_k)
    ...
    def _lexical_hits(
        ...
        """Return a bounded global BM25 complement without displacing cosine."""
        if self.lexical_top_k <= 0:
            return []
        eligible = [record for record in records if record.record_id not in excluded]
        scores = bm25_scores(query, eligible)
        ranked = sorted(
            (
                (score, record)
                for record in eligible
                if (score := scores.get(record.record_id, 0.0)) > 0.0
            ),
            key=lambda item: (-item[0], item[1].record_id),
        )
        return [
            RetrievalHit(record, "lexical", 0.0, float(score))
            for score, record in ranked[: self.lexical_top_k]
        ]

    @staticmethod
    def asks_for_action_evidence(query: str) -> bool:
        lowered = str(query).casefold()
        action = bool(
            re.search(
                r"\b(?:observed|noticed|noticing|inspected?|"
                r"read|ran|run|executed?|program|command|file|workspace|found)\b",
                lowered,
            )
            or re.search(
                r"\b(?:what|which)\b.{0,32}\bdid\b.{0,24}\b(?:i|you|we)\b.{0,16}\bdo\b",
                lowered,
            )
        )
        evidence = bool(
            re.search(
                r"\b(?:what|which|remember|already|previously|successfully|"
                r"actually|concrete|observed|verified)\b",
                lowered,
            )
        )
        return action and evidence

    def _verified_result_hits(
        self,
        query: str,
        query_embedding: Sequence[float],
        records: Sequence[MemoryRecord],
        ...
        if self.verified_result_top_k <= 0:
            return []
        projected: list[MemoryRecord] = []
        originals: dict[str, MemoryRecord] = {}
        abilities: dict[str, str] = {}
        for record in records:
            if record.record_id in excluded or record.record_type != RecordType.TOOL_RESULT:
                continue
            if not bool(record.metadata.get("verified")):
                continue
            if str(record.metadata.get("return_status")) != "success":
                continue
            rendered = render_verified_tool_result(record)
            embedding = self.graph.embedder.embed(rendered)
            projected.append(replace(record, text=rendered, embedding=tuple(embedding)))
            originals[record.record_id] = record
            try:
                abilities[record.record_id] = str(json.loads(record.text).get("ability", ""))
            except (TypeError, ValueError, json.JSONDecodeError):
                abilities[record.record_id] = ""
        lexical = bm25_scores(query, projected)
        ranked: list[tuple[float, RetrievalHit]] = []
        for projected_record in projected:
            dense = max(0.0, cosine_similarity(query_embedding, projected_record.embedding))
            lexical_score = lexical.get(projected_record.record_id, 0.0)
            if dense < self.direct_similarity_floor and lexical_score <= 0.0:
                continue
            ranked.append(
                (
                    0.55 * dense + 0.45 * lexical_score,
                    RetrievalHit(
                        originals[projected_record.record_id],
                        "verified",
                        dense,
                        lexical_score,
                    ),
        ...
        ranked.sort(key=lambda item: (-item[0], item[1].record.record_id))
        selected: list[RetrievalHit] = []
        used_abilities: set[str] = set()
        for _, hit in ranked:
            ability = abilities.get(hit.record.record_id, "")
            if ability in used_abilities:
                continue
            selected.append(hit)
            used_abilities.add(ability)
            if len(selected) >= self.verified_result_top_k:
                return selected
        for _, hit in ranked:
            if hit.record.record_id in {item.record.record_id for item in selected}:
                continue
            selected.append(hit)
            if len(selected) >= self.verified_result_top_k:
                break
    ...
    def retrieve(
        ...
        *,
        ...
        active_records = self.store.list_active_records()
        action_evidence_query = self.asks_for_action_evidence(query)
        records = [
            record
        ...
        verified = (
            self._verified_result_hits(
                query,
                query_embedding,
                active_records,
                excluded,
            )
            if action_evidence_query
            else []
        )
        ...
        lexical = self._lexical_hits(
            query,
            records,
            excluded | {hit.record.record_id for hit in direct},
        )
        ...
        for hit in (*verified, *direct, *lexical, *vault):
            if hit.record.record_id in seen:
                continue
            if hit.record.record_type == RecordType.THOUGHT:
                thought_key = hit.record.text.strip().casefold()
                if thought_key in seen_thoughts:
                    continue
                seen_thoughts.add(thought_key)
        ...
        packet = RetrievalPacket(
            ...
            lexical_record_ids=tuple(hit.record.record_id for hit in lexical),
